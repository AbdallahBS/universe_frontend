...
const SignupPage: React.FC<SignupPageProps> = ({ onSignupSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Show status modal before navigating to verify-email
      setPendingNavigation('/verify-email');
  ...
  const googleSignup = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        // Check if user has status set (new users won't have it)
        // Response is void, so we need to get user from context after login
        // We'll show status modal for new Google users
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="text-center">
                <p className="text-slate-600 dark:text-slate-400">
                  <TransText>{t("auth.haveAccount")}</TransText> <br></br>
                  <button
                    type="button"
                  ...
                  >
                    <TransText>{t("auth.signIn")}</TransText>
                  </button>
                </p>
              </div>
            </form>
